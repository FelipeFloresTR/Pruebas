...
Begin VB.Form FrmEmpresas 
   ...
   Begin VB.TextBox Txt_busqueda 
      ...
      MaxLength       =   15
      TabIndex        =   10
      Tag             =   "Ingrese Rut / Nombre Corto"
      Top             =   600
      Width           =   3975
   End
   Begin VB.CommandButton Bt_Seach 
      Caption         =   "&Buscar"
      Default         =   -1  'True
      Height          =   315
      Left            =   7200
...
Private Sub Bt_Seach_Click()
Call SeachLoadAll(Txt_busqueda)
End Sub
...
Private Sub SeachLoadAll(ByVal vTextBusqueda As String)
   Dim Q1 As String
   Dim Rs As Recordset
   Dim Row As Integer
   ...
   '3092122
   Else
       If Len(vTextBusqueda) > 0 Then
        If Op_SortRUT Then
             If ValidRut(vTextBusqueda) = False Then
                 MsgBox1 "Rut invalido, ingrese rut valido", vbExclamation
             Exit Sub
            End If
           End If
       
         Q1 = Q1 & " Where Estado = 0"
                       
          If gDbType = SQL_ACCESS Then
            If Op_SortRUT Then
                Q1 = Q1 & " AND RUT = '" & vFmtRut(vTextBusqueda) & "'"
            Else
                Q1 = Q1 & " AND NombreCorto like '*" & vTextBusqueda & "*'"
            End If
          Else
             If Op_SortRUT Then
                Q1 = Q1 & " AND RUT = '" & vFmtRut(vTextBusqueda) & "'"
