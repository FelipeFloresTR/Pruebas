...
'      Set RsSaldos = OpenRs(DbMain, "SELECT * FROM Cuentas WHERE 1=0", , dbOpenSnapshot)
   ...
   If TipoInforme = TI_TODOS Or TipoInforme = TI_SOLODOCS_SINCOMP Then
      ...
      'ffv 2855046
      Q1 = Q1 & "  , Documento.SaldoDoc,Documento.Tratamiento "
     ...
     ' Q1 = Q1 & " AND MovComprobante.idcuenta >0 " '2748525 - 2751484 OTROS DOCU
      ...
      Q1 = Q1 & "  Documento.TipoDoc, Documento.IdEntidad, Entidades.Nombre, Entidades.RUT, " & SqlInt("Entidades.NotValidRut") & ", "
      Q1 = Q1 & "  Documento.FEmisionOri, Documento.FVenc, Documento.Total, Documento.Descrip, "
      Q1 = Q1 & "  Documento.IdCtaBanco, Cuentas.Codigo, Cuentas.Descripcion, Documento.SaldoDoc, Documento.Tratamiento, Documento.IdDoc " '& DetComp1
   ...
   If TipoInforme = TI_TODOS Or TipoInforme = TI_SOLODOCS_CONCOMP Then
      ...
      'Q1 = Q1 & "  iif(Documento.SaldoDoc IS NOT NULL AND Documento.SaldoDoc <> 0 ,Sum(MovComprobante.Debe),Documento.Total) As Debe, iif(Documento.SaldoDoc IS NOT NULL AND Documento.SaldoDoc <> 0 ,Sum(MovComprobante.Haber),Documento.Total) As Haber, Documento.SaldoDoc"
      'Q1 = Q1 & "  iif(Documento.SaldoDoc > 0 AND Documento.Tratamiento = 1 , Documento.SaldoDoc, iif(Documento.Tratamiento = 1 AND (Documento.SaldoDoc IS NULL OR Documento.SaldoDoc = 0) ,Sum(MovComprobante.Debe),0)) As Debe, "
      'Q1 = Q1 & "  iif(Documento.SaldoDoc > 0 AND Documento.Tratamiento = 2 , Documento.SaldoDoc, iif(Documento.Tratamiento = 2 AND (Documento.SaldoDoc IS NULL OR Documento.SaldoDoc = 0) ,Sum(MovComprobante.Haber),0)) As Haber, "
      'Q1 = Q1 & "  iif(Documento.SaldoDoc <> 0 AND  Documento.SaldoDoc IS NOT NULL AND Documento.Tratamiento = 1 , Documento.SaldoDoc, iif(Documento.Tratamiento = 2 AND (Documento.SaldoDoc IS NULL OR Documento.SaldoDoc = 0) ,Sum(MovComprobante.Debe),Sum(MovComprobante.Haber))) As Debe, "
      'Q1 = Q1 & "  iif(Documento.SaldoDoc <> 0 AND  Documento.SaldoDoc IS NOT NULL AND Documento.Tratamiento = 2 , Documento.SaldoDoc, iif(Documento.Tratamiento = 1 AND (Documento.SaldoDoc IS NULL OR Documento.SaldoDoc = 0) ,Sum(MovComprobante.Haber),Sum(MovComprobante.Debe))) As Haber, "
...
'      End If
      ...
      If (lPorEntidad And IdEntidad <> vFld(Rs("IdEntidad"))) Or (Not lPorEntidad And IdCuenta <> vFld(Rs("IdCuenta"))) Then
         ...
         If IdEntidad <> 0 Or IdCuenta <> 0 Then    'la primera vez es cero
            ...
            If (Ch_SaldosVig <> 0 And Saldo <> 0) Or (Ch_SaldosVig = 0) Then
               ...
               Grid.TextMatrix(i, C_DEBE) = Format(Abs(SubTotDebe), NUMFMT)
               Grid.TextMatrix(i, C_HABER) = Format(Abs(SubTotHaber), NUMFMT)
               '2855046 ffv
               Grid.TextMatrix(i, C_SALDO) = Format(SubTotDebe - SubTotHaber, NEGNUMFMT)
               'Grid.TextMatrix(i, C_SALDO) = Format(Saldo, NEGNUMFMT)
               '2855046 ffv
      ...
      End If
       ...
       If vFld(Rs("Tratamiento")) = 1 And vFld(Rs("TotPagadoAnoAnt")) <> 0 And Abs(vFld(Rs("Debe"), True)) <> 0 Then
        Debe = Abs(vFld(Rs("Debe"), True))
        Haber = vFld(Rs("TotPagadoAnoAnt")) + Abs(vFld(Rs("Haber"), True))
       ElseIf vFld(Rs("Tratamiento")) = 2 And vFld(Rs("TotPagadoAnoAnt")) <> 0 And Abs(vFld(Rs("Haber"), True)) <> 0 Then
        Debe = Abs(vFld(Rs("Debe"), True)) * -1
        Haber = Abs(vFld(Rs("Haber"), True)) - (vFld(Rs("TotPagadoAnoAnt")))
        Else
        Debe = Abs(vFld(Rs("Debe"), True))
        Haber = Abs(vFld(Rs("Haber"), True))
       End If
...
'                  End If
         ...
         If CurIdDoc <> vFld(Rs("IdDoc")) Or (lPorEntidad And Ch_DetComp <> 0) Then  'si estamos presentando detalle por comprobante, separamos los movimientos de un mismo doc, para ver ambos comprobantes
            ...
            If CurIdDoc <> vFld(Rs("IdDoc")) And CurIdDoc > 0 And (lPorEntidad And Ch_DetComp <> 0) Then  'ponemos el total doc
               'ponemos el saldo doc
               Grid.TextMatrix(i, C_GLOSA) = "Saldo Documento"
               Grid.TextMatrix(i, C_SALDO) = Format(TotDebeParc - TotHaberParc, NEGNUMFMT)
               Grid.TextMatrix(i, C_OBLIGATORIA) = "O"
               Call FGrSetRowStyle(Grid, i, "FC", vbBlue)
               Grid.TextMatrix(i, C_FMT) = "B"
               Grid.rows = Grid.rows + 1
            ...
            Grid.TextMatrix(i, C_DEBE) = Format(Abs(Debe), BL_NUMFMT)
            Grid.TextMatrix(i, C_HABER) = Format(Abs(Haber), BL_NUMFMT)
            '2855046 ffv
...
'            SubTotHaber = Haber
         ...
         Else    'sin detalle comprobante
            ...
            i = i - 1
'2 vez entra aca
If Ch_InfRes = False Then
'        If Debe <> Abs(vFld(Rs("Total"))) And Haber <> 0 Then
'             SubTotDebe = SubTotDebe + Debe
'            Else
'             If Debe = Abs(vFld(Rs("Total"))) And Haber = 0 And CurIdDoc <> vFld(Rs("IdDoc")) Then
'             SubTotDebe = SubTotDebe + Debe
'             Else
'
'             If Debe = Abs(vFld(Rs("Total"))) And Haber <> 0 And CurIdDoc <> vFld(Rs("IdDoc")) Then
'             SubTotDebe = SubTotDebe + Debe
'
'             ElseIf Debe = Abs(vFld(Rs("Total"))) And Haber <> 0 Then
'                 'SubTotDebe = SubTotDebe + Debe
'                 SubTotDebe = Debe
'                 ElseIf Not Debe = Abs(vFld(Rs("Total"))) And Haber = 0 And CurIdDoc = vFld(Rs("IdDoc")) Then
'                 SubTotDebe = Debe
'                End If
'
'             End If
'         End If
' '2 vez entra aca
'         If Haber <> Abs(vFld(Rs("Total"))) And Debe <> 0 Then
'             SubTotHaber = SubTotHaber + Haber
'            Else
'             If Haber = Abs(vFld(Rs("Total"))) And Debe = 0 And CurIdDoc <> vFld(Rs("IdDoc")) Then
'             SubTotHaber = SubTotHaber + Haber
'             Else
'             If Haber = Abs(vFld(Rs("Total"))) And Debe <> 0 Then
'                 SubTotHaber = SubTotHaber + Haber
'             ElseIf Not Haber = Abs(vFld(Rs("Total"))) And Debe = 0 And CurIdDoc = vFld(Rs("IdDoc")) Then
'                 SubTotHaber = Haber
'              End If
'
'             End If
'         End If
          ...
          If lPorEntidad = False And vFld(Rs("IdDoc")) = CurIdDoc Then
 
            Grid.TextMatrix(i, C_DEBE) = Format(SubTotDebe, BL_NUMFMT)
            Grid.TextMatrix(i, C_HABER) = Format(SubTotHaber, BL_NUMFMT)
...
'1
         ...
         i = i + 1
      Else 'informe resumido
     '1 informe
            If Debe <> Abs(vFld(Rs("Total"))) And Haber <> 0 Then
             SubTotDebe = SubTotDebe + Debe
             TotDebeParc = TotDebeParc + Debe
            Else
             If Debe = Abs(vFld(Rs("Total"))) And Haber = 0 Then
             SubTotDebe = SubTotDebe + Debe
             TotDebeParc = TotDebeParc + Debe
 ...
 '2 informe
         If Haber <> Abs(vFld(Rs("Total"))) And Debe <> 0 Then
             SubTotHaber = SubTotHaber + Haber
              TotHaberParc = TotHaberParc + Haber
            Else
             If Haber = Abs(vFld(Rs("Total"))) And Debe = 0 And CurIdDoc <> vFld(Rs("IdDoc")) Then
             SubTotHaber = SubTotHaber + Haber
              TotHaberParc = TotHaberParc + Haber
...
'1 vez entra aca
         ...
         End If
          ...
          If vFld(Rs("IdDoc")) = CurIdDoc Then
 
            Grid.TextMatrix(i - 1, C_DEBE) = Format(SubTotDebe, BL_NUMFMT)
            Grid.TextMatrix(i - 1, C_HABER) = Format(SubTotHaber, BL_NUMFMT)
            
            'Grid.TextMatrix(i, C_SALDO) = Format(vFmt(Grid.TextMatrix(i, C_DEBE)) - vFmt(Grid.TextMatrix(i, C_HABER)), NEGNUMFMT)
            
            Grid.TextMatrix(i - 1, C_SALDO) = Format(SubTotDebe - SubTotHaber, NEGNUMFMT)
          End If
...
'  TotHaber = TotHaberParc
   ...
   If IdEntidad <> 0 Or IdCuenta <> 0 Then
      ...
      If CurIdDoc > 0 And lPorEntidad And Ch_DetComp <> 0 Then  'ponemos el total doc
         ...
         Grid.rows = Grid.rows + 1
         Grid.TextMatrix(i, C_GLOSA) = "Saldo Documento"
         Grid.TextMatrix(i, C_SALDO) = Format(vFmt(Grid.TextMatrix(i - 1, C_DEBE)) - vFmt(Grid.TextMatrix(i - 1, C_HABER)), NEGNUMFMT)
         Grid.TextMatrix(i, C_OBLIGATORIA) = "O"
         Call FGrSetRowStyle(Grid, i, "FC", vbBlue)
         Grid.TextMatrix(i, C_FMT) = "B"
         Grid.rows = Grid.rows + 1
      ...
      End If
      If lPorEntidad Then
          If Ch_DetComp Or Ch_InfRes Then
              Grid.TextMatrix(i, C_DEBE) = Format(Abs(SubTotDebe), NUMFMT)
              Grid.TextMatrix(i, C_HABER) = Format(Abs(SubTotHaber), NUMFMT)
              Grid.TextMatrix(i, C_SALDO) = Format(SubTotDebe - SubTotHaber, NEGNUMFMT)
          Else
              Grid.TextMatrix(i, C_DEBE) = Format(Abs(TotDebe), NUMFMT)
              Grid.TextMatrix(i, C_HABER) = Format(Abs(TotHaber), NUMFMT)
              Grid.TextMatrix(i, C_SALDO) = Format(TotDebe - TotHaber, NEGNUMFMT)
        End If
      Else
          Grid.TextMatrix(i, C_DEBE) = Format(Abs(SubTotDebe), NUMFMT)
          Grid.TextMatrix(i, C_HABER) = Format(Abs(SubTotHaber), NUMFMT)
   ...
   If lPorEntidad Then
    If Ch_DetComp Or Ch_InfRes Then
        GridTot.TextMatrix(0, C_DEBE) = Format(Abs(SubTotDebe), NUMFMT)
        GridTot.TextMatrix(0, C_HABER) = Format(Abs(SubTotHaber), NUMFMT)
        GridTot.TextMatrix(0, C_SALDO) = Format(SubTotDebe - SubTotHaber, NEGNUMFMT)
    Else
    
        GridTot.TextMatrix(0, C_DEBE) = Format(Abs(TotDebe), NUMFMT)
        GridTot.TextMatrix(0, C_HABER) = Format(Abs(TotHaber), NUMFMT)
        GridTot.TextMatrix(0, C_SALDO) = Format(TotDebe - TotHaber, NEGNUMFMT)
    End If
   ElseIf Ch_InfRes Then
     
        GridTot.TextMatrix(0, C_DEBE) = Format(Abs(TotDebeParc), NUMFMT)
        GridTot.TextMatrix(0, C_HABER) = Format(Abs(TotHaberParc), NUMFMT)
        GridTot.TextMatrix(0, C_SALDO) = Format(TotDebeParc - TotHaberParc, NEGNUMFMT)
   Else
