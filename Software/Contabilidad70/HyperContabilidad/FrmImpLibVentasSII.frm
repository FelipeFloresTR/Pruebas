VERSION 5.00
Begin VB.Form FrmImpLibVentasSII 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Importar Registro de Ventas SII (Formato CSV)"
   ClientHeight    =   4500
   ClientLeft      =   45
   ClientTop       =   360
   ClientWidth     =   10140
   ...
   Begin VB.CommandButton Bt_Manual 
      ...
      Height          =   795
      Left            =   6840
      Picture         =   "FrmImpLibVentasSII.frx":0000
      Style           =   1  'Graphical
      TabIndex        =   3
   ...
   Begin VB.PictureBox Picture1 
      ...
      Height          =   615
      Left            =   540
      Picture         =   "FrmImpLibVentasSII.frx":06B6
      ScaleHeight     =   615
      ScaleWidth      =   555
      TabIndex        =   11
      Top             =   600
   ...
   Begin VB.Frame Frame2 
      ...
      Begin VB.CommandButton Bt_Browse 
         Height          =   495
         Left            =   7800
         Picture         =   "FrmImpLibVentasSII.frx":0C43
         Style           =   1  'Graphical
         TabIndex        =   2
         Top             =   360
         Width           =   1215
   ...
   End
End
Attribute VB_Name = "FrmImpLibVentasSII"
...
'   End If
   ...
   Else
        ...
        If Me.Txt_UsuarioSII.Text = "" Then
   ...
   If Me.Op_Manual Then
        Call Import_LibroVentasSII(Me, lFName, Val(Tx_Ano), CbItemData(Cb_Mes))
   Else
        valida = True
        Info = InformacionSii(Val(Tx_Ano), CbItemData(Cb_Mes), Me.Txt_UsuarioSII.Text, Me.Txt_ClaveSII.Text)
        If valida Then
            Call Import_LibroVentasSIIAuto(Me, lFName, Val(Tx_Ano), CbItemData(Cb_Mes), Info)
        End If
   End If
...
Private Sub Bt_InfoAyuda_Click()
   Dim Frm As FrmInfoAyuda
   
   Set Frm = New FrmInfoAyuda
   Call Frm.FView("Tipos de Documentos de Venta")
   Set Frm = Nothing
End Sub
...
Private Sub Bt_Manual_Click()
   ...
   Buf = gAppPath & "\Manual_Registro_Ventas_SII.pdf"
   Rc = ExistFile(Buf)
      
   If Rc = 0 Then
      MsgBox1 "No se encontró el archivo que contiene el Manual de Importación del Registro de Ventas desde el SII, por favor contáctese con su proveedor para obtenerlo.", vbExclamation
   Else

      Rc = ShellExecute(Me.hWnd, "open", Buf, "", "", 1)
      If Rc < 32 Then
         MsgBox1 "Error " & Rc & " al abrir el archivo '" & Buf & "' que contiene el Importación del Registro de Ventas desde el SII." & vbLf & "Trate de abrir este archivo con otro programa.", vbExclamation
      End If
   End If
...
'********** INICIO ****************
   ...
   Params = "{" & Chr(34) & "metaData" & Chr(34) & ":{" & Chr(34) & "namespace" & Chr(34) & ":" & Chr(34) & "cl.sii.sdi.lob.diii.consdcv.data.api.interfaces.FacadeService/getDetalleVentaExport" & Chr(34) & "," & Chr(34) & "conversationId" & Chr(34) & ":" & Chr(34) & TOKEN_SII & Chr(34) & "," & Chr(34) & "transactionId" & Chr(34) & ":" & Chr(34) & Replace(Replace(s, "{", ""), "}", "") & Chr(34) & "," & Chr(34) & "page" & Chr(34) & ":null}," & Chr(34) & "data" & Chr(34) & ":{" & Chr(34) & "rutEmisor" & Chr(34) & ":" & Chr(34) & vFmtCID(Rut) & Chr(34) & "," & Chr(34) & "dvEmisor" & Chr(34) & ":" & Chr(34) & DV_Rut(vFmtCID(Rut)) & Chr(34) & "," & Chr(34) & "ptributario" & Chr(34) & ":" & Chr(34) & lAno & Format(lMes, "00") & Chr(34) & "," & Chr(34) & "codTipoDoc" & Chr(34) & ":0," & Chr(34) & "operacion" & Chr(34) & ":" & Chr(34) & "VENTA" & Chr(34) & "," & Chr(34) & "estadoContab" & Chr(34) & ":" & Chr(34) & "REGISTRO" & Chr(34) & "}}"
   Url = URL_SII_VENTA
