...
Public Sub TrasCuentas(DBSql As ADODB.Connection, DbAccess As Database, IdEmpresa As Long, Ano As Integer)
   ...
   Call CloseRs(Rs)
    ...
    Q1 = Q1 & " FROM Cuentas AS CU INNER JOIN Cuentas AS CUE ON CU.IDTRAS = CUE.idPadre"
...
Public Sub TrasMovComprobante(DBSql As ADODB.Connection, DbAccess As Database, IdEmpresa As Long, Ano As Integer)
   ...
   Call CloseRs(Rs)
    ...
    Q1 = Q1 & " LEFT JOIN Comprobante COM ON COM.IdTras = MV.IdComp AND COM.IdEmpresa = MV.IdEmpresa AND COM.Ano = MV.Ano)"
...
Public Sub TrasCuentasBasicas(DBSql As ADODB.Connection, DbAccess As Database, IdEmpresa As Long, Ano As Integer)
   ...
   Call CloseRs(Rs)
    ...
    Q1 = Q1 & "        CB.IdCuentaOld = ISNULL(CU.IdCuenta, Cb.IdCuentaOld)"
    Q1 = Q1 & " FROM CuentasBasicas CB"
    Q1 = Q1 & " LEFT JOIN Cuentas C  ON C.IdTras = CB.IdCuenta AND C.IdEmpresa = CB.IdEmpresa "
    Q1 = Q1 & " LEFT JOIN Cuentas CU ON CU.IdTras = CB.IdCuentaOld AND CU.IdEmpresa = CB.IdEmpresa"
...
Public Sub TrasSocios(DBSql As ADODB.Connection, DbAccess As Database, IdEmpresa As Long, Ano As Integer)
   ...
   Call CloseRs(Rs)
    ...
    Q1 = Q1 & "     S.IdCuentaRetiros = ISNULL(CU.IdCuenta, S.IdCuentaRetiros)"
    Q1 = Q1 & " FROM Socios S"
    Q1 = Q1 & " LEFT JOIN Cuentas C ON C.IdTras = S.IdCuentaAportes AND C.IdEmpresa = S.IdEmpresa AND C.Ano = S.Ano"
    Q1 = Q1 & " LEFT JOIN Cuentas CU ON CU.IdTras = S.IdCuentaRetiros AND CU.IdEmpresa = S.IdEmpresa AND CU.Ano = S.Ano"
...
Public Sub TrasCentroCosto(DBSql As ADODB.Connection, DbAccess As Database, IdEmpresa As Long, Ano As Integer)
   ...
   Do While Rs.EOF = False
            ...
            If Rs1.EOF = True Then
                ...
                Q1 = Q1 & " Values"
                Q1 = Q1 & " (" & IdEmpresa
                Q1 = Q1 & " ,'" & str(vFmt(vFldDao(Rs("Codigo")))) & "'"
                Q1 = Q1 & " ,'" & vFldDao(Rs("Descripcion")) & "'"
                Q1 = Q1 & " ," & vFldDao(Rs("Vigente"))
                Q1 = Q1 & " ," & vFldDao(Rs("IdCCosto")) & ")"
